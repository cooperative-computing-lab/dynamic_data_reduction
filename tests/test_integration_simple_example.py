...
def test_required_packages(test_env):
    ...
    required_packages = ["coffea", "ndcctools", "uproot", "rich", "dask", "fsspec-xrootd", "xrootd"]
    ...
    for package in required_packages:
        assert package.lower() in installed_packages, f"Package {package} not found in conda list"
...
def test_simple_example_runs(test_env):
    ...
    assert result.returncode == 0, f"Simple example failed with return code {result.returncode}"
